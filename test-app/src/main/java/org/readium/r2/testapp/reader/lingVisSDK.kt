...
fun attachLingVisSdk() { //tbd: move to LingVisSdk.kt
    CoroutineScope(Dispatchers.IO).launch {
        LingVisSDK.prepare("R2TestApp-Android", "r2 sample", "ef36f34a-5b6b-b58b-de98-9d634d1e01a6")
        R2App.server.loadCustomResource("window.lingVisSdk_Readium_isAndroid = true; ".byteInputStream(), "lingVisSdk-pre.js", Injectable.Script)
        withContext(Dispatchers.IO) {
            URL("https://sprakkraft.org/lingvis-sdk/readium/1.4.x/").openStream().use { inp ->
                R2App.server.loadCustomResource(inp,"lingVisSdk-poly-core.js", Injectable.Script
                )
            }
        }
    }
...
class LingVisSDK(val navigatorFragment: EpubNavigatorFragment?, val context: Context, val publication: Publication?) {
    ...
    companion object {
        fun prepare(app: String, clientData: String, clientId: String) {
            LingVisSDK.app = app
        ...
        var didExpire: (((String) -> Unit) -> Unit)? = null
        ...
        internal var autogenerated: Boolean = false
    }
    ...
    private fun attachToWebViews() {
        ...
        Timer().scheduleAtFixedRate(object : TimerTask() {
            override fun run() {
                ...
                val handler = LingVisHandler(that, webView, currentFragment.requireActivity(), false, bookId)
    ...
    suspend fun signIn(email: String, password: String, newAccount: Boolean, autogenerated: Boolean, clientKey: String): Result<String> = suspendCoroutine { cont ->
        ...
        mainWebView.evaluateJavascript("lingVisSdk.polyReadiumSignIn('${contId}', '', '${escape(email)}', '${escape(password)}'," +
                "'${escape(app)}', '', ${newAccount}, '', ${autogenerated}, '', '${clientKey}')", null)
    }
...
class LingVisHandler(val sdk: LingVisSDK, val webView: WebView, val context: Context, val isMain: Boolean, val bookId: String) {
    companion object {
        private var token = ""
    ...
    fun ready(args: String) {
        ...
        uiScope.launch {
            ...
            webView.evaluateJavascript(
                "lingVisSdk.polyReadiumSignIn('', '${token}', '${escape(LingVisSDK.email)}', '${escape(LingVisSDK.password)}', " +
                        "'${escape(LingVisSDK.app)}', '${id}', '', '${escape(LingVisSDK.clientData)}', " +
                        "${LingVisSDK.autogenerated}, '${LingVisSDK.clientId}', '', '${clientToken}', '${clientTokenTime}')", null
            )
        }
    }
    ...
    @android.webkit.JavascriptInterface
    fun expired(args: String) {
        if (LingVisSDK.didExpire == null) return
        fun refresh(clientKey: String) {
            uiScope.launch {
                webView.evaluateJavascript("lingVisSdk.polyReadiumRefresh('${clientKey}')", null)
            }
        }
